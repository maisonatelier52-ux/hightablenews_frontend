...
import { PAGE_BLOCK_DEFINITIONS, PAGE_BLOCK_LIBRARY_ORDER } from "@/lib/pageBlockDefinitions";

export default function PageBlockLibraryPanel({ onAddBlock }) {
  return (
    <div className="rounded-card border border-border bg-white shadow-soft overflow-hidden">
      <div className="px-4 py-3 border-b border-border bg-gray-50/50">
        <h3 className="text-[13.5px] font-semibold text-ink-900">Add a block</h3>
        <p className="text-[11.5px] text-ink-500 mt-0.5">Click any block to append it to the page.</p>
      </div>
      <div className="p-3 grid grid-cols-1 gap-2 max-h-[calc(100vh-320px)] overflow-y-auto">
        {PAGE_BLOCK_LIBRARY_ORDER.map((type) => {
          const def = PAGE_BLOCK_DEFINITIONS[type];
          const Icon = def.icon;
          return (
            <button
              key={type}
