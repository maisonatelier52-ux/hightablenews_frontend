...
import { articleHref, authorHref, getAllPreviewArticlesSorted } from "@/lib/articlesSource";
...
export default function StickyEditorialTemplate({ data, category, device = "desktop" }) {
  ...
  const mostReadCount = data.sidebar?.mostRead?.count ?? 3;
  const mostReadFromCategory = rest.slice(0, mostReadCount);
  const mostRead =
    mostReadFromCategory.length >= mostReadCount
      ? mostReadFromCategory
      : (() => {
          const usedIds = new Set([hero?.id, ...mostReadFromCategory.map((a) => a.id)].filter(Boolean));
          const fallback = getAllPreviewArticlesSorted().filter((a) => !usedIds.has(a.id));
          return [...mostReadFromCategory, ...fallback.slice(0, mostReadCount - mostReadFromCategory.length)];
        })();
  const latestCount = data.sidebar?.latest?.count ?? 4;
  const latestFromCategory = rest.slice(mostReadFromCategory.length, mostReadFromCategory.length + latestCount);
  // Small/newer categories can run out of real articles before the sidebar
  // is filled — top up from the site-wide pool (newest first) rather than
  // leaving the "Latest Updates" widget blank.
  const latest =
    latestFromCategory.length >= latestCount
      ? latestFromCategory
      : (() => {
          const usedIds = new Set([hero?.id, ...mostRead.map((a) => a.id), ...latestFromCategory.map((a) => a.id)].filter(Boolean));
          const fallback = getAllPreviewArticlesSorted().filter((a) => !usedIds.has(a.id));
          return [...latestFromCategory, ...fallback.slice(0, latestCount - latestFromCategory.length)];
        })();
  ...
  return (
    <aside className={stacked ? "w-full pt-6" : "pl-[22px] pt-1"} aria-label="Sidebar">
      ...
      {data.newsletter?.enabled && (
        <div className="mb-6 pb-6" style={{ borderBottom: `1px solid ${LINE}` }}>
          <div className="p-[18px] text-center" style={{ background: data.newsletter.bgColor || INK }}>
            <div className="flex justify-center mb-2" style={{ color: "#666666" }}><Mail size={22} /></div>
            <div className={`${HEAD} text-[1.0rem] font-bold mb-[6px] leading-[1.3]`} style={{ color: data.newsletter.headingColor || "#ffffff" }}>{data.newsletter.heading}</div>
            <div className={`${SANS} text-[0.72rem] mb-[14px] leading-[1.5]`} style={{ color: data.newsletter.subheadingColor || "#888888" }}>{data.newsletter.subheading}</div>
            <NewsletterForm
              source="category-sidebar"
              layout="stack"
              placeholder={data.newsletter.placeholder || "Your email address"}
              buttonText={data.newsletter.ctaLabel || "Subscribe Free"}
              successMessage={data.newsletter.successMessage || "You're subscribed! Please check your inbox."}
              buttonBg={data.newsletter.buttonColor || RED}
              buttonTextColor={data.newsletter.buttonTextColor || "#ffffff"}
              dark
            />
          </div>
        </div>
