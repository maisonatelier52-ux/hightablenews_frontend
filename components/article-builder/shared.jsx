    ...
    <div className={`flex items-center flex-wrap gap-1 text-[11.5px] ${dark ? "text-white/60" : "text-gray-400"}`}>
      {parts.map((p, i) => (
        <span key={i} className="flex items-center gap-1">
          {i > 0 && <ChevronRight size={11} />}
          {i === 0 ? (
            <Link href="/" title={p} className={dark ? "hover:text-white/90" : "hover:text-gray-600"}>{p}</Link>
          ) : (
            <span className={i === parts.length - 1 ? (dark ? "text-white/80" : "text-gray-600") : ""}>{p}</span>
          )}
        </span>
...
export function ByLine({ article, avatarSize = 32 }) {
  ...
  return (
    <div className="flex items-center gap-2.5">
      {href ? <Link href={href} title={article.author || "Staff Writer"}>{Avatar}</Link> : Avatar}
      <div>
        <p className="text-[12.5px] font-semibold text-gray-900 leading-tight">
          By {href ? <Link href={href} title={article.author || "Staff Writer"} className="hover:text-red-600 transition-colors">{article.author || "Staff Writer"}</Link> : (article.author || "Staff Writer")}
        </p>
        <p className="text-[11px] text-gray-400 flex items-center gap-1.5 mt-0.5">
          <span>{article.date}</span>
          {article.readTime && (
