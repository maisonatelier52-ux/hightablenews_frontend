...
function CardLink({ article, className, style, as: As = "div", children }) {
  ...
  return (
    <Link href={href} title={article?.title} className={className} style={style}>
      {children}
    </Link>
  );
}
    ...
    <>
      {articles.map((a, i) => (
        <span key={`${a.id}-${i}`} className="inline-flex items-center">
          {a.slug ? (
            <Link href={articleHref(a)} title={a.title} className="hover:underline underline-offset-2">{a.title}</Link>
          ) : (
            <span>{a.title}</span>
          )}
    ...
    case "stickyNotice":
      ...
      return (
        <div className="flex items-center justify-between gap-3 px-4 py-2.5" style={{ backgroundColor: data.bg || "#1a1a1a" }}>
          ...
          {data.ctaLabel && (
            <a href={data.ctaUrl} title={data.ctaLabel} className="shrink-0 text-[13px] font-bold px-3 py-1 rounded" style={{ backgroundColor: data.ctaBg || "#cc0000", color: "#fff" }}>{data.ctaLabel}</a>
          )}
        </div>
      );
    ...
    case "heroStory": {
      ...
      return (
        ...
        >
          ...
          >
            {heroUrl ? (
              <Link href={heroUrl} title={heroTitle} className="hover:underline underline-offset-4">{heroTitle}</Link>
            ) : (
              heroTitle
            )}
          </h2>
          ...
          {data.showCta && data.ctaLabel && heroUrl && (
            <Link href={heroUrl} title={data.ctaLabel} className="inline-block text-[14px] font-bold bg-red-600 text-white px-4 py-2 rounded hover:bg-red-700 transition-colors">{data.ctaLabel}</Link>
          )}
        </div>
      );
    }
      ...
      return (
        <div className="px-4 py-5" style={{ backgroundColor: data.bg || "#fff" }}>
          <div className="flex items-center gap-2 mb-4">
            ...
            {moreHref ? (
              <Link href={moreHref} title="More articles" className="text-[13px] font-medium text-red-600 hover:underline">More →</Link>
            ) : (
      ...
      return (
        <div className="px-4 py-3">
          {data.imageUrl && data.linkUrl ? (
            <a href={data.linkUrl} title={data.title || "Sponsored"} target="_blank" rel="noopener noreferrer sponsored" className="block" style={{ width: data.width > 0 ? data.width : "100%", maxWidth: "100%", marginInline: "auto" }}>
              {adInner}
            </a>
          ) : adInner}
        </div>
    ...
    case "fullWidthBanner":
      return (
        ...
        >
          ...
          {data.ctaLabel && (
            <a href={data.ctaUrl} title={data.ctaLabel} className="inline-block bg-white text-gray-900 text-[14px] font-bold px-4 py-2 rounded hover:bg-gray-100 transition-colors">{data.ctaLabel}</a>
          )}
        </div>
      );
          ...
          return (
            <div style={{ width: rW, maxWidth: "100%", marginInline: "auto" }}>
              {data.rightAdImage && data.rightAdLinkUrl ? (
                <a href={data.rightAdLinkUrl} title={data.rightAdTitle || "Sponsored"} target="_blank" rel="noopener noreferrer sponsored" className="block">{adBody}</a>
              ) : adBody}
            </div>
          );
        })()}
...
function EditableArticleSlot({ children, article, pinned, editable, onPick, onClear, className = "", style, excludeIds = [], currentArticleId = null }) {
  ...
  if (!editable) {
    ...
    if (href) {
      return (
        <Link href={href} title={article?.title} className={wrapClass} style={style}>
          {children}
        </Link>
      );
    }
  ...
  return (
    <div>
      ...
      <div className="pb-5 border-b border-gray-200 mb-5">
        ...
        <h2 className="font-black text-gray-900 leading-tight mb-4" style={{ fontFamily: "Georgia, serif", fontSize: heroHeadlineSz }}>
          ...
          <div>
            <span className="text-[13px] font-semibold text-gray-700">
              By {heroAuthorHref && !editable ? (
                <Link href={heroAuthorHref} title={heroArticle.author || data.heroAuthor || "James Whitmore"} className="hover:text-red-600 transition-colors">{heroArticle.author || data.heroAuthor || "James Whitmore"}</Link>
              ) : (
        ...
        return (
          <div key={section.id} className="mb-6 pb-6 border-b border-gray-100 last:border-0">
            <div className="flex items-center gap-2 mb-3">
              ...
              {sectionHref ? (
                <Link href={sectionHref} title="More articles" className="text-[12px] text-gray-400 font-medium hover:text-red-600 transition-colors cursor-pointer">More →</Link>
              ) : (
              ...
              <div key={block.id} className="border-t border-gray-200 pt-4">
                ...
                <div className="space-y-3">
                  {list.map((author, i) => {
                    ...
                    return href ? (
                      <Link href={href} key={author._id || author.name || i} title={author.name} className="flex items-center gap-2 group cursor-pointer">
                        {inner}
                      </Link>
                    ) : (
              ...
              <div key={block.id} className="border-t border-gray-200 pt-4">
                ...
                <div style={{ width: adW, maxWidth: "100%", marginInline: "auto" }}>
                  {block.imageUrl && block.linkUrl ? (
                    <a href={block.linkUrl} title={block.title || "Sponsored"} target="_blank" rel="noopener noreferrer sponsored" className="block">{adBody}</a>
                  ) : adBody}
                </div>
              </div>
            );
...
function AdBanner({ image, linkUrl, altText, width, height, sizeLabel, className = "", dark = false }) {
  ...
  return (
    <div className={className} style={{ width: w, maxWidth: "100%", marginInline: "auto" }}>
      {image && linkUrl ? (
        <a href={linkUrl} title={altText || "Sponsored"} target="_blank" rel="noopener noreferrer sponsored" className="block">{body}</a>
      ) : body}
    </div>
  );
}
        ...
        <div>
          ...
          <div style={{ width: wW, maxWidth: "100%", marginInline: "auto" }}>
            {widget.imageUrl && widget.linkUrl ? (
              <a href={widget.linkUrl} title={widget.title || "Sponsored"} target="_blank" rel="noopener noreferrer sponsored" className="block">{adBody}</a>
            ) : adBody}
          </div>
        </div>
      );
    ...
    <div className="min-w-0">
      ...
      {data.heroEnabled !== false && (
        ...
        >
          ...
          <h2 className="text-white font-bold leading-tight mb-3" style={{ fontSize: isMobile ? 24 : (data.heroTitleSize || 40) }}>
          ...
          {data.heroCtaLabel && (
            <a href={data.heroCtaUrl} title={data.heroCtaLabel} className="inline-block text-[14px] font-bold bg-white text-gray-900 px-4 py-2 rounded hover:bg-gray-100 transition-colors">{data.heroCtaLabel}</a>
          )}
        </div>
        </EditableArticleSlot>
      )}
      ...
      {data.topStoriesEnabled !== false && (
        <div className="mb-8">
          <div className="flex items-center gap-2 mb-4">
            ...
            <div className="flex-1 h-px bg-gray-200" />
            <a href="#" title="View all" className="text-[13px] font-medium text-red-600 hover:underline whitespace-nowrap">View All →</a>
          </div>
        ...
        return (
          <div key={cat.id} className="mb-8">
            <div className="flex items-center gap-2 mb-4">
              ...
              <div className="flex-1 h-px bg-gray-200" />
              {catHref ? (
                <Link href={catHref} title="View all articles" className="text-[13px] font-medium text-red-600 hover:underline whitespace-nowrap">View All →</Link>
              ) : (
          ...
          <div style={isMobile || isTablet ? undefined : { width: "75%", position: "sticky", top: 20, alignSelf: "flex-start" }} className="min-w-0">
            {data.heroEnabled !== false && (
              ...
              >
                ...
                <h2 className="text-white font-bold leading-tight mb-3" style={{ fontSize: isMobile ? 24 : (data.heroTitleSize || 42) }}>
                  {data.heroHeadline || heroArticle?.title || "No article yet — click + to choose one"}
                </h2>
                ...
                {data.heroCtaLabel && (
                  <a href={data.heroCtaUrl} title={data.heroCtaLabel} className="inline-block text-[14px] font-bold text-white px-4 py-2 rounded hover:opacity-90 transition-opacity" style={{ backgroundColor: accent }}>{data.heroCtaLabel}</a>
                )}
              </div>
              </EditableArticleSlot>
            )}
        ...
        {data.featuredStoriesEnabled !== false && (
          <div className="mt-8 mb-8">
            <div className="flex items-center gap-2 mb-4">
              ...
              <a href="#" title="View all" className="text-[13px] font-medium whitespace-nowrap hover:underline" style={{ color: accent }}>View All →</a>
            </div>
                ...
                return (
                  <div key={cat.id}>
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <div className="flex-1 h-px bg-white/10" />
                      {catHref ? (
                        <Link href={catHref} title="View all articles" className="text-[10.5px] font-medium whitespace-nowrap hover:underline" style={{ color: accent }}>View All →</Link>
                      ) : (
      ...
      <div className="mx-auto" style={{ maxWidth: data.maxWidth || 1600, padding: pad }}>
        ...
        <div className="mb-6" style={{ display: isMobile ? "flex" : "grid", flexDirection: isMobile ? "column" : undefined, gridTemplateColumns: isMobile ? undefined : "1fr 1fr", gap, gridTemplateRows: isMobile ? undefined : "auto" }}>
          ...
          >
            ...
            <h2 className="text-white font-bold leading-tight mb-2" style={{ fontSize: isMobile ? 24 : 36 }}>{data.heroHeadline || heroArticle?.title || "No article yet — click + to choose one"}</h2>
          ...
          <div className="mb-8">
            ...
            <div className="grid gap-5" style={{ gridTemplateColumns: `repeat(${isMobile ? 2 : Math.min(data.authorsCount || 4, 8)}, 1fr)` }}>
              {(getAuthors().length ? getAuthors() : AUTHORS_SAMPLE).slice(0, data.authorsCount || 4).map((author, i) => {
                ...
                return href ? (
                  <Link href={href} key={author._id || author.name || i} title={author.name} className="flex flex-col items-center text-center group">
                    {inner}
                  </Link>
                ) : (
