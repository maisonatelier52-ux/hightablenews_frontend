            ...
            {mostReadArticles.map((a, i, arr) => {
              ...
              return (
                <li key={a.id} className="flex gap-[10px] items-start py-[10px] group" style={{ borderBottom: i < arr.length - 1 ? `1px solid ${LINE}` : "none" }}>
                  ...
                  <div className="min-w-0">
                    <h4 className={`${HEAD} text-[0.8rem] font-bold leading-[1.28]`} style={{ color: INK }}>
                      <Item {...(href ? { href, title: a.title } : {})} className="hover:opacity-75">{a.title}</Item>
                    </h4>
                  </div>
                </li>
              );
