...
function socialLabel(platform) {
  const names = { twitter: "X (Twitter)" };
  return names[platform] || (platform ? platform.charAt(0).toUpperCase() + platform.slice(1) : "Social link");
}
...
function Logo({ logo, mobile, disableLinks }) {
  ...
  if (logo.type === "image" && logo.image) {
    return (
      <Link href="/" onClick={handleClick} title="Home" className="inline-flex items-center">
...
function HoverIcon({ icon: Icon, size, color, hoverColor, href, disableLinks, title }) {
...
function HoverLink({ children, href, color, hoverColor, className = "", disableLinks, title }) {
  const [hov, setHov] = useState(false);
      ...
      {items.map((a, i) => {
        ...
        return (
          <span key={`${a.id}-${i}`} className="inline-flex items-center">
            {href ? <Link href={href} title={a.title} className="hover:underline underline-offset-2">{a.title}</Link> : <span>{a.title}</span>}
  ...
  return (
    ...
    >
      ...
      {hasChildren && open && (
        <NavDropdownPortal coords={coords} onMouseEnter={show} onMouseLeave={scheduleHide}>
          {item.children.map((c) => (
            <Link key={c.id} href={c.url || "#"} onClick={handleChildClick} title={c.label} className="block px-3.5 py-2 text-[12px] hover:bg-black/5 cursor-pointer text-ink-700">{c.label}</Link>
          ))}
        </NavDropdownPortal>
      )}
    </div>
        ...
        <span className="flex items-center gap-1.5 text-ink-400">
          {socialPlatforms.map((p) => {
            ...
            return href && !disableLinks ? (
              <a key={p} href={href} target="_blank" rel="noopener noreferrer" title={socialLabel(p)} aria-label={socialLabel(p)} className="hover:text-ink-700 transition-colors">
                <Icon size={13} />
              </a>
            ) : (
              <Icon key={p} size={13} />
    ...
    <div className="border-b border-border/70">
      ...
      {hasChildren && expanded && (
        <div className="pb-2.5 pl-3 space-y-2">
          {item.children.map((c) => (
            <Link key={c.id} href={c.url || "#"} onClick={handleClick} title={c.label} className="block text-[12.5px] text-ink-500 py-1">{c.label}</Link>
          ))}
        </div>
      )}
    </div>
...
export default function HeaderPreview({ header, device = "desktop", disableLinks = false }) {
  ...
  // The header builder only lets the admin pick *which* platforms to show
  // as icons — the actual account URLs live on the Settings page's
  // "Social links" section (the single source of truth for social account
  // URLs). Pull that in so the header's social icons are real, clickable
  // links instead of dead decoration.
  useEffect(() => {
    let cancelled = false;
    getSocialLinksMap().then((map) => {
      if (!cancelled) setSocialLinks(map);
    });
    return () => {
      cancelled = true;
    };
                  ...
                  {rightSide.socialIcons && (
                    <span className="flex items-center gap-1.5">
                      {(rightSide.socialPlatforms || ["instagram", "twitter"]).map((p) => {
                        ...
                        return href && !disableLinks ? (
                          <a key={p} href={href} target="_blank" rel="noopener noreferrer" title={socialLabel(p)} aria-label={socialLabel(p)}>
