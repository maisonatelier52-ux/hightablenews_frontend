 ...
 *  that has never been opened/edited yet.
 *
 *  Exported (as hydrateCategoryPageConfig) so lib/serverApi.js can reuse the
 *  exact same defaulting logic when resolving the config on the server for
 *  the initial SSR pass of the public category page — the on-page template
 *  choice and its per-template settings need to be identical whether they
 *  were resolved server-side or client-side. */
function hydrate(config) {
  const templateId = config?.templateId && CATEGORY_TEMPLATE_DEFAULTS[config.templateId]
    ? config.templateId
    : DEFAULT_CATEGORY_PAGE.templateId;
...
export { hydrate as hydrateCategoryPageConfig };
