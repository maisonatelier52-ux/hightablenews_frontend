...
/** Synchronously seeds the cache with authors already fetched on the
 ...
 *  effect — so any child reading getAuthors() in that same render pass
 *  (e.g. the category-page Sidebar's "Correspondents" module, or the
 *  homepage's Authors block) sees real data immediately, on both the
 *  server-rendered HTML and the client render that hydrates it. Without
 *  this, those modules always rendered their "no authors yet" empty state
 *  in the first HTML response (all a crawler/Lighthouse ever sees), only
 *  populating a moment later once DataProvider's client-only preloadAuthors()
 *  effect resolved — a visible pop-in that also hurt CLS. */
export function seedAuthors(authors) {
  if (_loaded || !authors) return;
  _authors = (authors || []).map((a) => ({ ...a, _id: a._id }));
  _loaded = true;
