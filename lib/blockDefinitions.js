...
import { getCategories } from "./categoriesArticlesApi";
...
/**
 * Builds the default "category grid / category block" entries for a
 * template (Modern Magazine's Category Grid, Dark News's Category Blocks,
 * Masonry's Category Grid) by picking ONLY from categories that actually
 * exist on the Categories page — never a hardcoded name like "Business" or
 * "Politics" that may not exist on this site. A random subset (up to
 * `count`) is chosen each time the block is added, so the template doesn't
 * always show the same categories in the same order. If fewer real
 * categories exist than `count`, only that many entries are created (rather
 * than padding with fake ones). If NO categories exist yet at all (e.g. a
 * brand-new site), falls back to a couple of generic placeholder entries
 * with no `categoryId` so the settings panel still has something to show —
 * the admin can change these to real categories at any time in the
 * "Category Grid" / "Category Blocks" settings.
 */
function defaultCategoryEntries(count, { idPrefix, articleCount, fallbackNames = [] } = {}) {
  const real = typeof window !== "undefined" ? (getCategories() || []) : [];
  if (real.length > 0) {
    return shuffleArray(real).slice(0, count).map((cat, i) => ({
      id: `${idPrefix}${i + 1}`,
      categoryId: cat._id || "",
  ...
  return fallbackNames.slice(0, count).map((name, i) => ({
    id: `${idPrefix}${i + 1}`,
    categoryId: "",
    name,
    articleCount,
...
export const BLOCK_DEFINITIONS = {
  ...
  newspaperEditorial: {
    ...
    defaultData: () => ({
      ...
      rightBlocks: [
        ...
        { id: "rb6", type: "newsletter", label: "Newsletter", visible: false, enabled: false, title: "Newsletter", heading: "World Briefing", subheading: "Get the headlines that matter, delivered every morning.", placeholder: "Your email address", ctaLabel: "Subscribe", successMessage: "You're subscribed! Please check your inbox.", bgColor: "#111111", headingColor: "#FAFAF8", subheadingColor: "#888888", buttonColor: "#b91c1c", buttonTextColor: "#ffffff" },
      ],
  ...
  modernMagazineLayout: {
    ...
    defaultData: () => ({
      ...
      categoryGridCategories: defaultCategoryEntries(4, {
        idPrefix: "cg",
        articleCount: 4,
        fallbackNames: ["Business", "Technology", "Politics", "Lifestyle"],
      }),
  ...
  darkNewsLayout: {
    ...
    defaultData: () => ({
      ...
      categoryBlocks: defaultCategoryEntries(3, {
        idPrefix: "db",
        articleCount: 2,
        fallbackNames: ["World", "Business", "Technology"],
      }),
  ...
  masonryEditorialLayout: {
    ...
    defaultData: () => ({
      ...
      categoryGridCategories: defaultCategoryEntries(6, {
        idPrefix: "mc",
        articleCount: 4,
        fallbackNames: ["Business", "Technology", "Politics", "World", "Sports", "Lifestyle"],
      }),
...
/** Default settings for the category page's sticky-sidebar "Subscribe /
 *  Newsletter" box — admin can enable/disable it, edit all its copy, and
 *  customize its background, heading, subheading and button colors from
 *  the Category Page Builder. Submissions post to the same
 *  POST /api/subscribers endpoint used by every other newsletter form on
 *  the site. */
function defaultCategoryNewsletter() {
  return {
    enabled: true,
    heading: "World Briefing",
    subheading: "Our correspondents' dispatch, every morning at 7am.",
...
export const CATEGORY_TEMPLATE_DEFAULTS = {
  "sticky-editorial": () => ({
    ...
    sidebar: {
      ...
      newsletter: defaultCategoryNewsletter(),
...
function defaultAuthorSidebar() {
  return {
    ...
    newsletter: defaultAuthorNewsletter(),
    advertisement: { enabled: false },
  };
}

/** Default settings for the author page's sticky-sidebar "Subscribe /
 *  Newsletter" box — admin can enable/disable it, edit all its copy, and
 *  customize its background, heading, subheading and button colors from
 *  the Author Detail Page Builder. Submissions post to the same
 *  POST /api/subscribers endpoint used by every other newsletter form on
 *  the site. */
function defaultAuthorNewsletter() {
  return {
    enabled: true,
    heading: "World Briefing",
    subheading: "Our correspondents' dispatch, every morning at 7am.",
