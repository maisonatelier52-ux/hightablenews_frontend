...
//
// Exported (previously private) so callers that need this exact shape but
// must NOT depend on the shared module-level cache — e.g.
// ArticlePageClient resolving the CURRENT article straight from this
// render's own fresh server props — can normalize on demand instead of
// relying on seedCategoriesAndArticles(), which only ever runs once per
// page load and can go stale after a client-side navigation to a
// different article. See ArticlePageClient.jsx for details.
...
/** Synchronously seeds the cache with data that was already fetched on the
 *  server (see lib/serverApi.js) and passed down as props. Called from the
 *  very first render of a public page's client component — not from an
 *  effect — so that any child reading getCategories()/getArticles() in that
 *  same render pass (e.g. the homepage block renderer) sees real data
 *  immediately, on both the server-rendered HTML and the client render that
 *  hydrates it. This is what lets crawlers and Lighthouse see full content
 *  (headings, article titles, word count) in the first response instead of
 *  a loading skeleton.
 *
 *  Marks the loader as already-resolved so the site-wide DataProvider (see
 *  components/site/DataProvider.jsx) doesn't kick off a redundant duplicate
 *  fetch a moment later. This is intentionally a ONE-TIME seed — it will
 *  NOT refresh the cache on a later call (e.g. when navigating client-side
 *  to a different article page, which passes a new, different
 *  initialArticles set). That's fine for shared widgets like related-
 *  articles/sidebar lists, which only need lightweight preview data and
 *  are fine holding onto slightly-stale entries — but it means the shared
 *  cache must never be used to resolve the CURRENT page's own primary
 *  content (e.g. the article body being read right now); that should
 *  always come straight from that render's own fresh props instead. */
export function seedCategoriesAndArticles(categories, articles) {
  if (_loaded || !categories || !articles) return;
  _categories = normalizeCategories(categories);
  _articles = normalizeArticles(articles);
  _loaded = true;
  _loadingPromise = Promise.resolve();
  notify();
}
...
export async function saveArticle(data) {
  // `status` is now the source of truth (draft / published / scheduled),
  // sent straight from the form's Status field. `isPublished` is kept as a
  // fallback for any older caller that might still only pass that boolean.
  ...
  const payload = {
    ...
    status,
    // Only meaningful (and only sent) when the article is actually
    // scheduled — avoids sending a stale date along with a draft/published
    // save.
    scheduledFor: status === "scheduled" ? data.scheduledFor : undefined,
  };
