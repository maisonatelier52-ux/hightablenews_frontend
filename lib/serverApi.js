...
//   // Same idea, for the article detail page: fetches the (single, shared)
//   // article-page-builder config on the server so
//   // app/(site)/[category]/[slug]/page.jsx can render the real template —
//   // the real <h1> article title, real <h2>/<h3> body/related headings — in
//   // the very first HTML response instead of a client-only skeleton. This is
//   // what was missing (unlike the homepage/category pages, which already did
//   // this) and is the direct cause of the "missing <h1>", "0% title/H1
//   // coherence", and very low FCP/LCP/TTI scores from the SEO/Lighthouse
//   // audit: crawlers and Lighthouse only read/measure this first response.
//   getArticleDetailPageConfig: async () => {
//     const { hydrateArticleDetailPage } = await import("./articleDetailPageApi");
//     const saved = await safeGet("/article-detail-page-config", { revalidate: 10 });
//     return hydrateArticleDetailPage(saved);
//   },
// };
...
export const serverApi = {
  ...
  // Same idea, for the author profile page: fetches the (single, shared)
  // author-page-builder config on the server so
  // app/(site)/author/[slug]/page.jsx can render the real template — the
  // real <h1> author name, real bio/<h2> headings, real article list — in
  // the very first HTML response instead of a client-only skeleton.
  getAuthorPageConfig: async () => {
    const { hydrateAuthorPage } = await import("./authorPageApi");
    const saved = await safeGet("/author-page-config", { revalidate: 10 });
    return hydrateAuthorPage(saved);
  },
};