...
//     meta: "Est. 1998 · London · New York · Singapore",
//     font: "blackletter",
//     size: 56,
//     color: "#ffffff",
...
// // Header
// export function mergeHeader(saved) {
...
// // Footer
// export function mergeFooter(saved) {
...
import { headerApi as headerAdmin, footerApi as footerAdmin, homepageApi as homepageAdmin } from "@/apis/adminApis";
import { headerApi as headerPublic, footerApi as footerPublic, homepageApi as homepagePublic, settingsApi as settingsPublic } from "@/apis/usersideApis";
...
export const DEFAULT_FOOTER = {
  ...
  // Just the ordered list of *selected* platform keys now — not
  // {platform, url} pairs. The actual account URL for each platform is
  // managed in one place, the Settings page's "Social links" section (see
  // lib/socialPlatforms.js + getSocialLinksMap() below), so it can't drift
  // out of sync between the Header Builder and Footer Builder.
...
/**
 * Normalizes a saved footer/header `social` value into a clean, ordered
 * list of plain platform-key STRINGS (e.g. ["twitter", "linkedin"]).
 *
 * Footer/header configs saved before the Settings-based social-links
 * refactor stored `social` as a list of legacy `{ platform, url }` objects.
 * Loading one of those old configs straight into the Footer/Header
 * Builder used to pass those raw objects into `socialPlatformLabel()` /
 * `socialPlatformIcon()`, which expect a string — causing
 * "key.charAt is not a function" the moment the builder tried to render
 * the social icon picker or list.
 *
 * This unwraps `.platform` from any legacy object entries, drops
 * anything that isn't a usable string, and removes duplicates —
 * so both old and new saved data end up in the same shape.
 */
function normalizeSocialList(value) {
  ...
  for (const entry of value) {
    ...
    if (typeof entry === "string") {
      key = entry;
    } else if (entry && typeof entry === "object" && typeof entry.platform === "string") {
      key = entry.platform; // legacy { platform, url } shape
    }
    key = key ? key.trim() : "";
...
export function mergeHeader(saved) {
  ...
  return {
    ...
    topBar: {
      ...
      ...(saved.topBar || {}),
      socialPlatforms: normalizeSocialList(
        (saved.topBar || {}).socialPlatforms ?? DEFAULT_HEADER.topBar.socialPlatforms
      ),
    },
    rightSide: {
      ...DEFAULT_HEADER.rightSide,
      ...(saved.rightSide || {}),
      socialPlatforms: normalizeSocialList((saved.rightSide || {}).socialPlatforms),
    },
    breakingNews: { ...DEFAULT_HEADER.breakingNews, ...savedBreakingNews },
    mobile: { ...DEFAULT_HEADER.mobile, ...(saved.mobile || {}) },
    menuStyle: { ...DEFAULT_HEADER.menuStyle, ...(saved.menuStyle || {}) },
...
export function mergeFooter(saved) {
  ...
  return {
    ...
    branding: { ...DEFAULT_FOOTER.branding, ...(saved.branding || {}) },
    newsletter: { ...DEFAULT_FOOTER.newsletter, ...(saved.newsletter || {}) },
    // `social` may be legacy [{platform,url}] data from before the
    // Settings-based refactor — normalize it to a plain string array so
    // the builder's SocialEditor and the public preview never receive
    // anything but platform-key strings.
    social: normalizeSocialList(saved.social ?? DEFAULT_FOOTER.social),
    bottomBar: { ...DEFAULT_FOOTER.bottomBar, ...(saved.bottomBar || {}) },
    depth: { ...DEFAULT_FOOTER.depth, ...(saved.depth || {}) },
    mobile: { ...DEFAULT_FOOTER.mobile, ...(saved.mobile || {}) },
  };
...
//
// Settings → "Social links" is the single source of truth for social
// account URLs. The Header Builder and Footer Builder only let the admin
// pick *which* of those platforms to show as icons — they don't store a
// URL themselves. This resolves the admin-entered list into a simple
// { platform: url } map, keeping only entries that actually have a URL, so
// callers can do `map[platform]` and skip a platform entirely if it has no
// link configured yet.
export async function getSocialLinksMap() {
  try {
    const settings = await settingsPublic.get();
    const map = {};
    (settings?.socialLinks || []).forEach((s) => {
      if (s?.platform && s?.url) map[s.platform] = s.url;
    });
    return map;
  } catch {
    return {};
