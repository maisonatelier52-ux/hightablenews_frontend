...
// Shows a live "42/60" style counter under SEO fields and turns amber/red
// once the text runs past the length search engines will actually display
// (~60 chars for a title, ~155-160 for a description) — this is what the
// "Meta Description is too long" / "H1 too short" style warnings from SEO
// auditing tools are checking for, so surfacing it while typing here means
// it never has to be caught after deploying.
function LengthCounter({ length, min, max }) {
  const tooShort = min && length > 0 && length < min;
  const tooLong = length > max;
  const color = tooLong ? "text-red-500" : tooShort ? "text-amber-500" : "text-ink-400";
  return (
    <p className={`text-[11px] mt-1 ${color}`}>
      {length}/{max} characters
      {tooLong && " — this will likely be truncated by Google. Shorten it."}
      {tooShort && ` — a bit short; aim for ${min}-${max}.`}
    </p>
  );
}
                ...
                <FormSection title="SEO Settings">
                  <FormField label="SEO Title" required error={formErrors.seoTitle}>
                    <input
                      ...
                      placeholder="Custom page title for search engines"
                    />
                    <LengthCounter length={formData.seoTitle.length} min={30} max={60} />
                  </FormField>
                  <FormField label="SEO Description" required error={formErrors.seoDescription}>
                    <textarea
                      value={formData.seoDescription}
                      ...
                      placeholder="Custom meta description for this category page"
                    />
                    <LengthCounter length={formData.seoDescription.length} min={50} max={160} />
                  </FormField>
                </FormSection>
